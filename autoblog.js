...
export async function runAutoblog(appType) {
  ...
  if (appType === 'iptv') {
    ...
    prompt = `
      ...
      Каждый раз придумывай новый сюжет для картинки (например: телевизор в темноте, пульт в руке, попкорн на фоне экрана, семья на диване и т.д.).

      Ответ ОБЯЗАТЕЛЬНО должен быть в таком формате:
      IMAGE_PROMPT: [Короткое описание на английском языке для генератора картинок. Опиши уникальный сюжет. В конец добавь: photorealistic, amateur photo, taken on mobile phone, real life, cozy living room, no text]
      TEXT:
      [Сам текст поста]
