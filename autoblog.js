...
export async function runAutoblog(appType) {
  ...
  if (appType === 'iptv') {
    ...
    prompt = `
      ...
      IMAGE_PROMPT: [Короткое описание на английском языке для генератора картинок. СТРОГОЕ ПРАВИЛО: НИКАКИХ ЛЮДЕЙ, ЛИЦ ИЛИ РУК В КАДРЕ! Только интерьеры, телевизоры, техника. В конец добавь: photorealistic, 4k, cozy room, no people, no text]
      TEXT:
      [Сам текст поста]
